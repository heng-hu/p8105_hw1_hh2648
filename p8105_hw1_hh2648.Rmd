...
`visib` is the visibility in miles. It is a critical parameter for airport operations. The mean visibility in this dataset is ``r round(mean(early_january_weather$visib), 1)`` miles, with a standard deviation of ``r round(sd(early_january_weather$visib), 1)`` miles.

`wind_speed` is the speed of wind in MPH. It is another critical parameter for airport operations. The mean wind speed in this dataset is ``r round(mean(early_january_weather$wind_speed), 1)`` MPH, with a standard deviation of ``r round(sd(early_january_weather$wind_speed), 1)`` MPH.

In winter, temperature is also important for the airport. The mean temperature in early Januray, `r yr` is ``r round(sd(early_january_weather$temp), 1)`` F.
